...
| M2a | Consolidation by similarity absorption | **failed**: −53 pp | **failed**, reproduced: 6.0× compression, −35.2 pp (seed 42), −36.4 pp (seed 7); seed 1337 running | compression ≥ 5× ∧ Δacc ≥ −1 pp: **not met** (as predicted) |
| M2b | Utility-based eviction + L2 persistence gate | crash replay passed | **gate passed** (seeds 42 and 7; 1337 running): full replay, 10/10 checkpoints and a crash replay at a 60 % byte cut reproduce state and all predictions exactly; eviction to 80 % costs −5.99 pp (seed 42), −5.49 pp (seed 7) | persistence gate: **met** |
| M3 | 1,000 authors, strictly sequential | 5.94 % top-1; state invariant, read-out **not** (W19) | top-1 **14.61 %**, top-5 27.44 %, forgetting +21.1 pp (seed 42; seeds 7 and 1337 running); **state and read-out invariance both hold** (0 differences); reconstruction from the log exact | forgetting ≤ 1 pp, top-1 ≥ 30 %: **not met** (as predicted) |
