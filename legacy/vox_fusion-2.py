...
class VoxFusion:
    ...
    def turn(self, user: str):
        ...
        return reply, {"erinnert": [f for f, _ in facts],
                       "neu_gelernt": learned,
...
def cmd_demo(seed=42):
    ...
    for u in DEMO:
        ...
        print(f"  DU : {u}")
        print(f"  VOX: {reply}")
        if prov["erinnert"] or prov["neu_gelernt"]:
            print(f"       [erinnert: {prov['erinnert']} | "
                  f"neu: {prov['neu_gelernt']} | {prov['abruf_ms']} ms]")
        print()
...
def cmd_chat(seed=42):
    ...
    while True:
        ...
        tags = []
        if prov["erinnert"]:
            tags.append(f"erinnert: {', '.join(prov['erinnert'])}")
        if prov["neu_gelernt"]:
            tags.append(f"gelernt: {', '.join(prov['neu_gelernt'])}")
        if tags:
            print(f"     [{' | '.join(tags)} | {prov['abruf_ms']} ms]")
    print("\nBis bald! Gedächtnis lebt bis Prozessende "
          "(Persistenz via L2-Log = nächster Schritt).")
