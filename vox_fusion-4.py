...
class Memory:
    ...
    def __init__(self, seed: int = 42, persist_path: str | None = "data/vox_facts.log",
        ...
        self._keys = None
        self.extractor = extractor          # optionaler LLM-Extraktor (None -> Regex)
    ...
    def _clean_value(val: str) -> str:
        """Wert an Konjunktionen/Satzgrenzen abschneiden (Regex-Fallback)."""
        val = val.strip(" .,!?")
        val = re.split(r"\s+(?:and|und)\s+|,", val, maxsplit=1)[0].strip()
        return " ".join(val.split()[:4])

    def _extract(self, text: str):
        """Fakten als (subject, value) extrahieren. LLM-Extraktor bevorzugt
        (versteht 'ich bin 15', 'wohne in X und Y' korrekt); Regex als Fallback."""
        if self.extractor is not None:
            try:
                facts = self.extractor(text)          # [{"subject":..,"value":..}, ...]
                out = [(f["subject"].strip().lower(), f["value"].strip())
                       for f in facts
                       if f.get("subject") and f.get("value")]
                if out:
                    return out
            except Exception:
                pass                                  # fällt auf Regex zurück
        # --- Regex-Fallback (subject = Kategorie/Sachwort) ---
        out, seen = [], set()
        for pat, cat in self.FACT_PATTERNS:
            for m in pat.finditer(text):
                ...
                if cat == "live":
                    ...
                    parts = raw.split()               # "Hund Loui" -> subj=Hund, val=Loui
                    if len(parts) >= 2:
                        out.append((parts[0].lower(), self._clean_value(" ".join(parts[1:]))))
                    continue
                else:
                    val = self._clean_value(raw)
                key = (cat, val.lower())
                if len(val) >= 2 and key not in seen:
                    seen.add(key)
                    out.append((cat, val))
        return out

    # Subject-Synonyme: verbinden Frageworte mit dem gespeicherten Fakt-Subject
    SUBJECT_HINTS = {
        "alter": "alter alt jahre age old wie alt",
        "wohnort": "wohnort wohne wo ort live where stadt",
        "hund": "hund dog haustier tier name heißt",
        "name": "name heiße heißt called wer bin ich",
        "beruf": "beruf arbeite job work",
        "vorliebe": "mag liebe like love essen food",
    }

    def absorb(self, text: str):
        """Neue Fakten schreiben (append-only). Fakt = 'subject: value';
        Abrufschlüssel = subject-Synonyme + value, damit auch Zahlen (Alter)
        über das Frage-Subject gefunden werden."""
        new = []
        for subject, value in self._extract(text):
            fact = f"{subject}: {value}"
            if fact in self.facts:
                continue
            hint = self.SUBJECT_HINTS.get(subject, subject)
            key_text = f"{hint} {value}"       # Frageworte dominieren den Schlüssel
...
# ---------------------------------------------------------------------------
# FAKT-EXTRAKTOR (LLM) — löst die Regex-Grenzen ('ich bin 15', 'X und Y')
# ---------------------------------------------------------------------------
EXTRACT_PROMPT = (
    "Extrahiere dauerhafte Fakten über den NUTZER aus seiner Nachricht. "
    "Nur bleibende Eigenschaften (Name, Alter, Wohnort, Beruf, Haustier, "
    "Vorlieben) — keine Smalltalk-Sätze, keine Fragen. Antworte NUR als "
    'JSON-Liste: [{"subject":"alter","value":"15"}]. Wenn keine Fakten: [].\n'
    'Beispiel: "Ich bin 15 und wohne in Dortmund und Marbella" -> '
    '[{"subject":"alter","value":"15"},'
    '{"subject":"wohnort","value":"Dortmund und Marbella"}]\n'
    "Nachricht: ")


class QwenExtractor:
    """Nutzt dasselbe lokale Qwen wie der Mund, um saubere Fakten zu ziehen."""

    def __init__(self, llm):
        self.llm = llm                      # geteilte Llama-Instanz (kein 2. Modell)

    def __call__(self, text: str):
        import json
        prompt = (f"<|im_start|>system\nDu bist ein Fakt-Extraktor.<|im_end|>\n"
                  f"<|im_start|>user\n{EXTRACT_PROMPT}{text}<|im_end|>\n"
                  f"<|im_start|>assistant\n")
        out = self.llm(prompt, max_tokens=150, temperature=0.0,
                       stop=["<|im_end|>"])["choices"][0]["text"].strip()
...
class MockExtractor:
    """Deterministischer Extraktor für Container-Verifikation (netzfrei).
    Bildet nach, was ein fähiges LLM aus den Testsätzen zöge."""

    RULES = [
        (re.compile(r"\bbin (\d{1,3})\b", re.I), "alter"),
        (re.compile(r"\b(?:i am|i'm) (\d{1,3})\b", re.I), "alter"),
        (re.compile(r"\b(?:mein hund heißt|my dog is called) ([\w-]+)", re.I), "hund"),
        (re.compile(r"\b(?:ich heiße|my name is) ([\w-]+)", re.I), "name"),
        (re.compile(r"\bwohne (?:in|zwischen) ([\w äöü]+?)(?: und bin\b| und \d| bin\b|\.|!|$)", re.I), "wohnort"),
        (re.compile(r"\bich (?:mag|liebe) ([\w äöü-]+?)(?:\.|!|$)", re.I), "vorliebe"),
    ]

    def __call__(self, text: str):
        out = []
        for pat, subj in self.RULES:
            m = pat.search(text)
            if m:
                out.append({"subject": subj, "value": m.group(1).strip()})
        return out


# ---------------------------------------------------------------------------
class VoxFusion:
    def __init__(self, mouth: MouthBackend, seed: int = 42, extractor=None):
        self.mem = Memory(seed, extractor=extractor)
        self.mouth = mouth
...
def cmd_selftest(seed=42):
    # Testet die LLM-Extraktor-Bahn mit exakt Eriks Live-Bug-Sätzen
    m = Memory(seed, persist_path=None, extractor=MockExtractor())
    m.absorb("Mein Hund heißt Loui")
    m.absorb("Ich wohne in Marbella und Dortmund und bin 15 Jahre alt")
    m.absorb("Ich heiße Erik")
    checks = [("wie heißt mein hund", "Loui"),
              ("wie alt bin ich", "15"),
              ("wo wohne ich", "Marbella"),
              ("wie heiße ich", "Erik")]
    ok = True
    print("VOX-Fusion Selftest (LLM-Extraktor-Bahn, netzfrei):")
    print(f"  Gelernte Fakten: {m.facts}")
...
def cmd_chat(seed=42):
    ...
    if model:
        ...
        extractor = QwenExtractor(mouth.llm)   # dieselbe Modell-Instanz, kein 2. Laden
        print("VOX-Fusion — Qwen (Mund + Fakt-Extraktor) + ENGRAMM-Gedächtnis.\n")
    else:
        mouth = MockMouth()
        extractor = MockExtractor()
        print("VOX-Fusion — Qwen nicht gefunden, nutze Mock-Mund + Mock-Extraktor.\n"
              "(Für den echten Mund: Modell nach models/ und in .venv_b1 starten.)\n")
    v = VoxFusion(mouth, seed, extractor=extractor)
